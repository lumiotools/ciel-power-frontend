...
export default function ProposalsSection({ bookingNumber }: ProposalsSectionProps) {
  ...
  const fetchContracts = async () => {
    ...
    try {
      const response = await fetch(`/api/user/bookings/${bookingNumber}/contracts`)
  ...
  return (
    <>
      <PortalSection id="review-plans" icon={<Clock className="text-[#8bc34a]" size={24} />} title="Your Proposals">
        <h3 className="text-lg font-medium text-gray-600 mb-4 ml-9">Review Plans & Say Go</h3>

        <p className="text-gray-700 mb-4">
          Review plans, and find what works for you! Once you have signed off on the contract, we'll go ahead and line
          everything up. Keeping you informed at every step of the journey.
        </p>
        ...
        <div className="bg-white rounded-lg p-5 mb-4 border border-[#e0f0d0]">
          ...
          {!isLoading && !error && contracts.length === 0 && (
            <div className="mt-4 p-6 bg-gray-50 rounded-lg border border-gray-200 text-center">
              ...
              <p className="text-gray-600">No contracts available at this time.</p>
                    ...
                    <tr key={contract.id} className="border-b border-gray-200">
                      ...
                      <td className="py-3 px-4">
                        {contract.signed && contract.downloadLink ? (
                          <button
                            className="text-[#8bc34a] font-medium hover:underline flex items-center"
                            onClick={() => handleDownloadContract(contract.downloadLink!)}
                          >
                            Download <span className="ml-1">›</span>
                          </button>
                        ) : !contract.signed && allowSigning ? (
                          <button
                            className="bg-[#8bc34a] text-white px-3 py-1 rounded-md hover:bg-[#7cb342]"
                          ...
                          <span className="text-gray-400">
                            {contract.signed ? "Download unavailable" : "Signing unavailable"}
                          </span>
                        )}
                      </td>
                    </tr>
          ...
          <p className="mt-4">All signed documents are stored securely and can be downloaded anytime.</p>
        </div>
      </PortalSection>
