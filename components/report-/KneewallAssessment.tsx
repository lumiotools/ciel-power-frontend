...
export function KneewallAssessment({
  ...
  const processedKneewallData = (): KneewallData[] => {
    ...
    const kneewallItem: KneewallData = {
      ...
      recommendation: (data.name || "Your Kneewall Insulation")
        ?.toLowerCase()
        .includes("flat")
        ? "R60"
        : "R13", // Default recommendations based on type
      currentValue: data.rValue,
      maxValue: (data.name || "Your Kneewall Insulation")
        ?.toLowerCase()
        .includes("flat")
        ? 60
        : 13,
    ...
    //   condition: data.condition || "Not assessed",
    //   rValue: data.rValue ? `R${data.rValue}` : "Not assessed",
    //   recommendation: (data.name || "Your Kneewall Flat Insulation")
    //     ?.toLowerCase()
    //     .includes("flat")
    //     ? "R60"
    //     : "R13", // Default recommendations based on type
    //   currentValue: data.rValue,
    //   maxValue: (data.name || "Your Kneewall Flat Insulation")
    //     ?.toLowerCase()
    //     .includes("flat")
    //     ? 60
    //     : 13,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            ...
            <div className="bg-[#F7FDFC] p-2">
              ...
              <div className="mt-5 p-4 bg-[#E0F7F5] rounded-lg">
                <h3 className="font-medium mb-2 text-[#256C68]">
                  BPI Recommendation
                </h3>
                <p className="text-sm text-gray-700">
                  BPI recommends{" "}
                  {item.title.includes("Flat") ? "Kneewall Flats" : "Kneewalls"}{" "}
                  be insulated to {item.recommendation} for optimal energy
                  efficiency. Your current insulation is at {item.rValue}, which
                  is {Math.round((item.currentValue / item.maxValue) * 100)}% of
