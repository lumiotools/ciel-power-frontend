...
export function InsulationZoneAssessment({ data, isAdmin = false, onUpdate }: InsulationZoneProps) {
  ...
  const updateZoneData = (field: keyof typeof zoneData, value: string | number) => {
    setZoneData((prev) => {
      ...
      } else if ((field === "material" || field === "condition") && onUpdate) {
    ...
    return (
      <div className="relative w-full max-w-[300px] mx-auto">
        <svg viewBox="-10 0 220 120" className="w-full">
          ...
          {[0, Math.round(maxValue/4), Math.round(maxValue/2), Math.round(maxValue*3/4), maxValue].map((markerValue, index) => (
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              {isAdmin ? (
                <ImageUpload
                  src={zoneData.image || "/placeholder.svg"}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="text-sm text-gray-500 dark:text-gray-400">Current R-Value</div>
                    <div className="font-medium mt-1 text-gray-900 dark:text-gray-100">
                      {isAdmin ? (
                        <EditableField
                    ...
                    <div className="font-medium mt-1 text-gray-900 dark:text-gray-100">
                      {isAdmin ? (
                        <EditableField
                          ...
                          onSave={(value) => updateZoneData("recommendedValue", Number.parseInt(value.replace("R", "")) || 13)}
                          type="text"
                        />
                      ) : (
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="text-gray-900 dark:text-gray-100">
                        {isAdmin ? (
                          <EditableField
                            value={`${Math.min(Math.round((zoneData.rValue / zoneData.recommendedValue) * 100), 100)}`}
                            onSave={(value) => {
                              const efficiency = Number.parseInt(value)
                              if (!isNaN(efficiency)) {
                                const newRValue = Math.round((efficiency * zoneData.recommendedValue) / 100)
