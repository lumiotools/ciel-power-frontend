  ...
  return (
    <div className="relative w-full max-w-[400px] mx-auto">
      <svg viewBox="-10 0 220 110" className="w-full">
        ...
        <text x="100" y="95" textAnchor="middle" className="text-4xl font-bold" fill="#b45309">
          {value}
        </text>
      </svg>
    </div>
...
const InPlaceEdit: React.FC<InPlaceEditProps> = ({ initialValue, isAdmin, onUpdate, label = null }) => {
  ...
  return (
    <div className="text-xl font-bold text-amber-700 flex items-center cursor-pointer" onClick={handleClick}>
      {value}{label ? ` ${label}` : ''}
...
const CoolingSystemCard: React.FC<CoolingSystemCardProps> = ({ 
  ...
  const [systemData, setSystemData] = useState<SystemData>({
    ...
    currentSEER: typeof item.value === 'number' ? item.value : 10,
    recommendedSEER: 16,
            ...
            <div className="space-y-6">
              <div ref={itemRef} className="relative bg-[#fffbeb] dark:bg-amber-900/20 rounded-lg p-6">
                ...
                <div className="mt-8 grid grid-cols-2 gap-8">
                  <div>
                    <p className="text-base text-gray-600 dark:text-gray-400">Current {item.parameter}</p>
                    {isAdmin && onUpdateItem ? (
                      ...
                      <p className="text-3xl font-bold text-[#b45309] dark:text-amber-400">{systemData.currentSEER}</p>
                    )}
                  </div>
                  <div>
                    <p className="text-base text-gray-600 dark:text-gray-400">BPI Recommends</p>
                    <p className="text-3xl font-bold text-[#65a30d] dark:text-green-400">{systemData.recommendedSEER}</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="space-y-6">
              <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-amber-700 dark:text-amber-300">System Details</h3>
                </div>
                <p className="text-gray-600 dark:text-gray-300 mb-4">
                  Your air conditioner&apos;s current {item.parameter} rating of {systemData.currentSEER} means it&apos;s operating
                ...
                <div className="flex items-center space-x-2 text-amber-600 mt-4">
                  ...
                  <span className="text-sm">*SEER = Seasonal Energy Efficiency Ratio</span>
                </div>
              </div>
              ...
              {isAdmin && onUpdateItem ? (
                ...
                <motion.div className="relative h-48 overflow-hidden rounded-lg" whileHover={{ scale: 1.02 }}>
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4">
                    <p className="text-white text-sm">Example of a {systemData.type || "central"} air conditioning unit</p>
                  </div>
                </motion.div>
              )}
            </div>
...
export function CoolingContent({ data, isAdmin = false, onUpdateItem }: CoolingContentProps) {
  const fadeInUp = {
    initial: { opacity: 0, y: 20 },
    animate: { opacity: 1, y: 0 },
  ...
  const coolingItems = data?.data?.filter(item => 
