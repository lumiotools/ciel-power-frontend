...
export default function PercentageGauge({
  ...
  const calculateRotation = (value: number): number => {
    // Define the angles for different percentage values
    const angles = {
      0: 118.17,    // 0%
      25: 90,       // 25%
      50: 75,       // 50%
    ...
    // Find the two closest percentage values
    const percentages = Object.keys(angles).map(Number)
    const lowerPercentage = Math.max(...percentages.filter(p => p <= value))
    const upperPercentage = Math.min(...percentages.filter(p => p >= value))
    
    // If value exactly matches a defined percentage, return its angle
    if (lowerPercentage === upperPercentage) {
      return angles[lowerPercentage]
    }
    
    // Interpolate between the two angles
    const lowerAngle = angles[lowerPercentage]
    const upperAngle = angles[upperPercentage]
    const percentage = (value - lowerPercentage) / (upperPercentage - lowerPercentage)
