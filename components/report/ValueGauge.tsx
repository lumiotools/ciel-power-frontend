...
export default function ValueGauge({
  ...
  }, [animatedValue, minValue, maxValue, unit])
  ...
  const drawGauge = (
    ...
    // For efficiency values, higher is better
  ...
  const drawTickMarks = (
    ...
    const majorTicks = [minValue, minValue + tickStep, minValue + tickStep * 2, minValue + tickStep * 3, maxValue]
  ...
  const drawValueText = (
    ...
    // Draw value with optional unit
    ctx.font = `bold ${centerX * 0.18}px Arial`
    ctx.fillStyle = "#b45309"
    ctx.fillText(`${Math.round(value)}${unit}`, centerX, textY + centerX * 0.28)
  }
